...
* 手动 ModItem 组装与编辑（成员角色、封面、元数据）。✅ Task 3 完成（成员角色、元数据）；封面设置待后续
* 安全移动预演、确认、结果与撤销 UI。⬜ Task 5

明确不做：搜索、AI JSON、联网预览图、缩略图生成、卡片拖拽、文件监听、增量扫描、压缩包解析、MO2 集成、跨设备同步、打包发布。

验收：

* 用户可添加、查看、移除受管理根目录配置。（添加/查看已实现；移除未实现）
* 用户可手动触发扫描，看到扫描结果、错误摘要与未关联素材。（扫描结果、错误摘要与未关联素材池已实现）
* 用户可在素材池选择文件创建 ModItem，设置成员角色与封面。（成员角色已实现；封面设置待后续）
