...
使用方式（由 ThumbnailCoordinator 调度）：
    thread = QThread()
    worker = ThumbnailWorker(db_path, thumbnails_dir, unit_id, source_path)
...
class ThumbnailWorker(QObject):
    ...
    信号：
    - thumbnail_ready(str, str)：生成完成。(unit_id, status)
      status 取值与 ThumbnailCache.status 一致（'ok' / 'missing' / 'corrupt' /
      'unsupported' / 'error'）。status='ok' 表示缓存命中可用，其他表示失败。
    - thumbnail_failed(str, str)：生成过程抛出未预期异常。(unit_id, error_message)
    """

    thumbnail_ready = Signal(str, str)  # unit_id, status
    thumbnail_failed = Signal(str, str)  # unit_id, error_message
    ...
    def run(self) -> None:
        ...
        try:
            ...
            status = service.generate(self._unit_id, self._source_path)
            conn.commit()
            self.thumbnail_ready.emit(self._unit_id, status)
        except Exception as e:  # noqa: BLE001
            logger.exception("缩略图 worker 异常：unit_id=%s", self._unit_id)
            self.thumbnail_failed.emit(self._unit_id, str(e))
        finally:
            if conn is not None:
                try:
                    conn.close()
