...
class ThumbnailCoordinator(QObject):
    ...
    def request_thumbnail(
        ...
        - 命中：返回缩放后的 QPixmap（已缩放到 self._size）
        - 未命中：返回 None，并把任务加入队列（去重）
        """
        cache_path = self._service.get_cache(content_unit_id, source_path)
        if cache_path is not None:
            return self._load_pixmap(cache_path)
        # 未命中 → 投递队列（去重）
        self._enqueue(content_unit_id, source_path)
        return None

    def invalidate(self, content_unit_id: str) -> None:
        """失效指定内容单元的缓存（封面清除/更换时调用）。"""
    ...
    def _enqueue(self, content_unit_id: str, source_path: Path) -> None:
        """投递生成任务到队列（去重）。"""
        self._mutex.lock()
        try:
            if content_unit_id in self._pending:
                return  # 已在队列或处理中
            self._pending.add(content_unit_id)
            self._queue.append((content_unit_id, source_path))
        finally:
            self._mutex.unlock()
        self._dispatch_next()
    ...
    def _dispatch_next(self) -> None:
        ...
        if self._thread is not None or not self._queue:
            self._mutex.unlock()
            return
        unit_id, source_path = self._queue.popleft()
        self._mutex.unlock()
        self._start_worker(unit_id, source_path)

    def _start_worker(self, unit_id: str, source_path: Path) -> None:
        """启动 worker 处理指定任务。"""
    ...
    def _load_pixmap(self, path: Path) -> QPixmap | None:
        ...
        if pixmap.isNull():
            logger.warning("加载缩略图 PNG 失败：%s", path)
            return None
        return pixmap
