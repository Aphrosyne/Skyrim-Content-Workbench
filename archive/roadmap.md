...
目标：让用户可以用图形界面完成一次真实、可预演、可恢复的本地 Mod 整理流程。详见 [docs/phase-2-plan.md](phase-2-plan.md)。
...
* 三栏浏览工作台：未关联素材池、ModItem 列表与详情。✅ Task 3 完成
* 手动 ModItem 组装与编辑（成员角色、封面、元数据）。✅ Task 3 完成（成员角色、元数据）；✅ Task 4 完成（封面设置与缩略图预览）
* 本地缩略图缓存与 ModItem 预览图展示。✅ Task 4 完成
* 安全移动预演、确认、结果与撤销 UI。⬜ Task 5

明确不做：搜索、AI JSON、联网预览图、卡片拖拽、文件监听、增量扫描、压缩包解析、MO2 集成、跨设备同步、打包发布。

验收：

* 用户可添加、查看、移除受管理根目录配置。（添加/查看/移除已实现）
* 用户可手动触发扫描，看到扫描结果、错误摘要与未关联素材。（扫描结果、错误摘要与未关联素材池已实现）
* 用户可在素材池选择文件创建 ModItem，设置成员角色与封面。（成员角色与封面设置已实现）
* 用户可将预览图成员设为封面，在列表与详情区看到缩略图。（Task 4 已实现）
* 用户可在目录树选择目标分类，发起移动预演并明确确认执行。
* 用户可看到执行结果（成功、失败、阻止原因），对安全操作发起撤销预演并确认撤销。
* 中文路径、中文显示名和 UTF-8 数据在扫描、展示、组装、移动与撤销流程中保持可用。
